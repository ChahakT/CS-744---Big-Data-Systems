...
int main(void) {
    ...
    for (size_t i = 0; i < size; i++) {
        int *value = reinterpret_cast<int*>(malloc(sizeof(int)* inputEle));
    ...
    gloo::ScatterOptions opts(k_context);
    if (rank == size -1) {
        opts.setInputs(inputPointers, inputEle);
    }
    for (int i = 0; i < inputEle; i++) {
        outputPointers[i] = 0;
    }
...
//    auto allgather = std::make_shared<gloo::AllgatherRing<int>>(
//                    k_context, ptrs, outPtr, inputEle);
//
//    // Run the algorithm.
//    allgather->run();
//
...
//        std::cout << "data[" << i << "] = " << outPtr[i] << std::endl;
//    }
    return 0;
}