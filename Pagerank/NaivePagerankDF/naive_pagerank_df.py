# Group 23 Assignment 1 Pagerank Code (Naive, DF)
...
def hashtag_filter(line):
    """
    Filter out data that start with '#'
    """
...
def get_links_and_ranks_dataframes(spark_session: SparkSession, input_file: str):
    ...
    Linkn -> 1.0
    """
    data_df_unfiltered = spark_session.read.text(input_file)
    data_df = data_df_unfiltered.filter(
        hashtag_filter(data_df_unfiltered['value']))
    ...
    links_df = \
        split_data_df\
        .groupBy('FromNodes')\
        .agg(collect_list("ToNodes").alias('ToNodes'))
...
def run_naive_pagerank_iterations(spark_session: SparkSession, input_file: str, num_iterations: int = 10):
    ...
    for _ in range(num_iterations):
        ...
        ranks_df = contribs.withColumn("Rank", col("sum(Rank)") * 0.85 + 0.15)
...
def run(spark_master:str, input_file: str, output_dir: str, num_iterations: int = 10):
    ...
    final_ranks = run_naive_pagerank_iterations(spark_session, input_file, 1)
