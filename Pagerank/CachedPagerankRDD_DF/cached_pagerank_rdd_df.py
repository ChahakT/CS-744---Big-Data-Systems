# Group 23 Assignment 0 Pagerank Code (Cached, DF)
...
def get_links_and_ranks_dataframes(spark_session: SparkSession, input_file: str):
    ...
    links_df = \
        split_data_df\
        .groupBy('FromNodes')\
        .agg(collect_list("ToNodes").alias('ToNodes')).cache()
...
def run_cached_pagerank_iterations(spark_session: SparkSession, input_file: str, num_iterations: int = 10):
    """
...
def run(spark_master:str, input_file: str, output_dir: str, num_iterations: int = 10):
    ...
    final_ranks = run_cached_pagerank_iterations(spark_session, input_file, num_iterations)
